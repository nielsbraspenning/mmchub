...
class TennetSoap extends SoapClient
{
    ...
    public function __doRequest(string $request, string $location, string $action, int $version, bool $one_way = false): ?string
    {
        ...
        $bodyElement = generateEnergyAccountBody([
            'mRID' => 'DOC-FCR-23112025-1A-001',
            'revisionNumber' => 1,
            'senderId' => '8719333027500',
            'receiverId' => '8716867999983',
            'createdDateTime' => '2025-11-24T07:30:00',
            'periodStart' => '2025-11-23T00:00:00',
            'periodEnd' => '2025-11-24T00:00:00',
            'timeseriesEnd' => '2025-11-23T08:00:00',
            'timeSeriesId' => 'TS-20251123-CC',
      ...
      //  $key->loadCert($this->signingCert); 
       // $key->loadCert(file_get_contents($this->signingCert));
...
function extractSKIfromCert(string $certPath): string {
    ...
    if (!isset($parsed['extensions']['subjectKeyIdentifier'])) {
        throw new Exception("SKI not found in certificate");
    }

    // Convert from hex (e.g., "7F:1D:82...") to raw binary and then base64
    $hex = str_replace(':', '', $parsed['extensions']['subjectKeyIdentifier']);
    $bin = hex2bin($hex);
